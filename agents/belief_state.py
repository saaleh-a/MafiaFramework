...
# Default threshold for Self-Preservation Protocol: if a Detective/Doctor's
# perceived suspicion exceeds this, they should reveal their identity to survive.
SELF_PRESERVATION_THRESHOLD = 0.45

# Graduated Self-Preservation Protocol thresholds
SPP_SOFT_HINT_THRESHOLD = 0.35     # Soft reveal hint
SPP_HARD_CLAIM_THRESHOLD = 0.45    # Hard role claim with conditional
SPP_FULL_REVEAL_THRESHOLD = 0.55   # Immediate full reveal

# Red-check adjustment: if Detective holds confirmed Mafia finding,
# lower all thresholds by this amount (information > survival)
SPP_RED_CHECK_ADJUSTMENT = 0.10
...
class SuspicionState:
    ...
    def should_reveal_identity(self, own_name: str, all_beliefs: dict[str, "SuspicionState"]) -> bool:
        """
        Self-Preservation Protocol: return True if enough other agents suspect *own_name*
    ...
    def get_spp_level(
        ...
        *, has_red_check: bool = False,
        ...
        """
        Graduated Self-Preservation Protocol: return the appropriate reveal level.
        ...
        adjustment = SPP_RED_CHECK_ADJUSTMENT if has_red_check else 0.0

        if avg >= SPP_FULL_REVEAL_THRESHOLD - adjustment:
            return "full_reveal"
        if avg >= SPP_HARD_CLAIM_THRESHOLD - adjustment:
            return "hard_claim"
        if avg >= SPP_SOFT_HINT_THRESHOLD - adjustment:
            return "soft_hint"
        return None
