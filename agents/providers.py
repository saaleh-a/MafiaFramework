...
class BeliefStateProvider(ContextProvider):
    ...
    Lifeline Protocol reveal instructions into each agent call.
    ...
    State keys used in session.state["belief"]:
        ...
        all_beliefs:    dict[str, SuspicionState] (for Lifeline Protocol)
        role:           str role name
    ...
    async def before_run(
        ...
        *,
        ...
        # Lifeline Protocol: graduated identity reveal
        ...
        if role in ("Detective", "Doctor") and all_beliefs:
            ...
            lifeline_level = suspicion.get_lifeline_level(
                name, all_beliefs, has_red_check=has_red_check,
            )
            if lifeline_level == "soft_hint":
                context.extend_instructions(
                    self.source_id,
                    f"⚠ LIFELINE PROTOCOL (SOFT HINT): You ({name}) are attracting "
            ...
            elif lifeline_level == "hard_claim":
                context.extend_instructions(
                    self.source_id,
                    f"⚠ LIFELINE PROTOCOL (HARD CLAIM): Suspicion against you "
            ...
            elif lifeline_level == "full_reveal":
                context.extend_instructions(
                    self.source_id,
                    f"⚠ LIFELINE PROTOCOL (FULL REVEAL): You ({name}) are about "
